...
import debugLogData from "./debugLogData.event.js";
import setRoomState from "./debugSetRoomState.event.js";
...
export const handleQuizBattleEvents = (socket) => {
    verifySocketJWT(socket, () => {
        ...
        socket.on("kickPlayer", (userID, callback) => handleKickPlayer(socket, userID, callback));
        socket.on("isRoomExisting", (roomID) => handleIsRoomExisting(socket, roomID));
        socket.on("markQuestion", (index) => handleMarkQuestion(socket, index));
        socket.on("revealQuestion", (categoryIndex, questionIndex) => handleRevealQuestion(socket, categoryIndex, questionIndex));
        socket.on("revealAnswer", (categoryIndex, questionIndex, roomID) => handleRevealAnswer(socket, categoryIndex, questionIndex, roomID));
        socket.on("toggleReveal", (categoryIndex, questionIndex) => handleToggleReveal(socket, categoryIndex, questionIndex));
        socket.on("endQuestion", () => handleEndQuestion(socket));
        socket.on("backToBoard", () => handleBackToBoard(socket));
        socket.on("buzzerPress", (callback) => handleBuzzerPress(socket, callback));
        socket.on("correctBuzzerAnswer", () => handleMarkBuzzerCorrect(socket));
        socket.on("wrongBuzzerAnswer", () => handleMarkBuzzerWrong(socket));
        socket.on("skippingPress", () => handleSkippingPress(socket));
        socket.on("changeScoreOfPlayer", (changeScoreState) => handleChangeScoreOfPlayer(socket, changeScoreState));
        ...
        socket.on("debugLogData", (callback) => debugLogData(socket, callback));
        socket.on("debugSetRoomState", (roomID, state) => setRoomState(socket, roomID, state));
    });
};
